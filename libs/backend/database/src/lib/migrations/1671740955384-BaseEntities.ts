...
export class BaseEntities1671740955384 implements MigrationInterface {
  name = 'BaseEntities1671740955384';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`version\` (\`id\` varchar(36) NOT NULL, \`major\` int UNSIGNED NOT NULL, \`minor\` int UNSIGNED NOT NULL, \`isCurrent\` tinyint NOT NULL DEFAULT 0, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_f02a2968cc0da218cc97a9fd5a\` (\`major\`, \`minor\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
