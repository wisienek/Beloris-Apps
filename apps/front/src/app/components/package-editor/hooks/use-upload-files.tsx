...
import { usePackageCreator } from './use-package-creator';

export const useUploadFiles = () => {
  const { addError } = useContext(ErrorContext);
  const { version, files, isCurrentVersion, isPackage } =
  ...
  const { createPackage } = usePackageCreator();
  ...
  const uploadFiles = async () => {
    ...
    if (isPackage) {
      const packageInfo = await createPackage(version);
